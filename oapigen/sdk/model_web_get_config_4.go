/*
Morpheus API

Morpheus is a powerful cloud management tool that provides provisioning, monitoring, logging, backups, and application deployment strategies.  This document describes the Morpheus API protocol and the available endpoints. Sections are organized in the same manner as they appear in the Morpheus UI.

API version: 9.0.0
Contact: dev@morpheusdata.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sdk

import (
	"encoding/json"
)

// checks if the WebGetConfig4 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &WebGetConfig4{}

// WebGetConfig4 struct for WebGetConfig4
type WebGetConfig4 struct {
	// HTTP method to use for testing
	WebMethod string `json:"webMethod"`
	// Web URL you wish to use to run a check on
	WebUrl string `json:"webUrl"`
	// Ignore SSL Errors
	IgnoreSSL *bool `json:"ignoreSSL,omitempty"`
	// If you want to use HTTP Basic Authentication, populate this field with the username
	CheckUser *string `json:"checkUser,omitempty"`
	// If you want to use HTTP basic Authentication, populate this field with the password
	CheckPassword *string `json:"checkPassword,omitempty"`
	// Set value to `on` if you want to turn on text matching
	TextCheckOn *string `json:"textCheckOn,omitempty"`
	// Set the string you want to look for in the page source
	WebTextMatch         *string                `json:"webTextMatch,omitempty"`
	AdditionalProperties map[string]interface{} `json:",remain"`
}

type _WebGetConfig4 WebGetConfig4

// NewWebGetConfig4 instantiates a new WebGetConfig4 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewWebGetConfig4(webMethod string, webUrl string) *WebGetConfig4 {
	this := WebGetConfig4{}
	this.WebMethod = webMethod
	this.WebUrl = webUrl
	var ignoreSSL bool = false
	this.IgnoreSSL = &ignoreSSL
	return &this
}

// NewWebGetConfig4WithDefaults instantiates a new WebGetConfig4 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewWebGetConfig4WithDefaults() *WebGetConfig4 {
	this := WebGetConfig4{}
	var ignoreSSL bool = false
	this.IgnoreSSL = &ignoreSSL
	return &this
}

// GetWebMethod returns the WebMethod field value
func (o *WebGetConfig4) GetWebMethod() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.WebMethod
}

// GetWebMethodOk returns a tuple with the WebMethod field value
// and a boolean to check if the value has been set.
func (o *WebGetConfig4) GetWebMethodOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.WebMethod, true
}

// SetWebMethod sets field value
func (o *WebGetConfig4) SetWebMethod(v string) {
	o.WebMethod = v
}

// GetWebUrl returns the WebUrl field value
func (o *WebGetConfig4) GetWebUrl() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.WebUrl
}

// GetWebUrlOk returns a tuple with the WebUrl field value
// and a boolean to check if the value has been set.
func (o *WebGetConfig4) GetWebUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.WebUrl, true
}

// SetWebUrl sets field value
func (o *WebGetConfig4) SetWebUrl(v string) {
	o.WebUrl = v
}

// GetIgnoreSSL returns the IgnoreSSL field value if set, zero value otherwise.
func (o *WebGetConfig4) GetIgnoreSSL() bool {
	if o == nil || IsNil(o.IgnoreSSL) {
		var ret bool
		return ret
	}
	return *o.IgnoreSSL
}

// GetIgnoreSSLOk returns a tuple with the IgnoreSSL field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebGetConfig4) GetIgnoreSSLOk() (*bool, bool) {
	if o == nil || IsNil(o.IgnoreSSL) {
		return nil, false
	}
	return o.IgnoreSSL, true
}

// IsSetIgnoreSSL returns a boolean if a field has been set.
func (o *WebGetConfig4) IsSetIgnoreSSL() bool {
	if o != nil && !IsNil(o.IgnoreSSL) {
		return true
	}

	return false
}

// SetIgnoreSSL gets a reference to the given bool and assigns it to the IgnoreSSL field.
func (o *WebGetConfig4) SetIgnoreSSL(v bool) {
	o.IgnoreSSL = &v
}

// GetCheckUser returns the CheckUser field value if set, zero value otherwise.
func (o *WebGetConfig4) GetCheckUser() string {
	if o == nil || IsNil(o.CheckUser) {
		var ret string
		return ret
	}
	return *o.CheckUser
}

// GetCheckUserOk returns a tuple with the CheckUser field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebGetConfig4) GetCheckUserOk() (*string, bool) {
	if o == nil || IsNil(o.CheckUser) {
		return nil, false
	}
	return o.CheckUser, true
}

// IsSetCheckUser returns a boolean if a field has been set.
func (o *WebGetConfig4) IsSetCheckUser() bool {
	if o != nil && !IsNil(o.CheckUser) {
		return true
	}

	return false
}

// SetCheckUser gets a reference to the given string and assigns it to the CheckUser field.
func (o *WebGetConfig4) SetCheckUser(v string) {
	o.CheckUser = &v
}

// GetCheckPassword returns the CheckPassword field value if set, zero value otherwise.
func (o *WebGetConfig4) GetCheckPassword() string {
	if o == nil || IsNil(o.CheckPassword) {
		var ret string
		return ret
	}
	return *o.CheckPassword
}

// GetCheckPasswordOk returns a tuple with the CheckPassword field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebGetConfig4) GetCheckPasswordOk() (*string, bool) {
	if o == nil || IsNil(o.CheckPassword) {
		return nil, false
	}
	return o.CheckPassword, true
}

// IsSetCheckPassword returns a boolean if a field has been set.
func (o *WebGetConfig4) IsSetCheckPassword() bool {
	if o != nil && !IsNil(o.CheckPassword) {
		return true
	}

	return false
}

// SetCheckPassword gets a reference to the given string and assigns it to the CheckPassword field.
func (o *WebGetConfig4) SetCheckPassword(v string) {
	o.CheckPassword = &v
}

// GetTextCheckOn returns the TextCheckOn field value if set, zero value otherwise.
func (o *WebGetConfig4) GetTextCheckOn() string {
	if o == nil || IsNil(o.TextCheckOn) {
		var ret string
		return ret
	}
	return *o.TextCheckOn
}

// GetTextCheckOnOk returns a tuple with the TextCheckOn field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebGetConfig4) GetTextCheckOnOk() (*string, bool) {
	if o == nil || IsNil(o.TextCheckOn) {
		return nil, false
	}
	return o.TextCheckOn, true
}

// IsSetTextCheckOn returns a boolean if a field has been set.
func (o *WebGetConfig4) IsSetTextCheckOn() bool {
	if o != nil && !IsNil(o.TextCheckOn) {
		return true
	}

	return false
}

// SetTextCheckOn gets a reference to the given string and assigns it to the TextCheckOn field.
func (o *WebGetConfig4) SetTextCheckOn(v string) {
	o.TextCheckOn = &v
}

// GetWebTextMatch returns the WebTextMatch field value if set, zero value otherwise.
func (o *WebGetConfig4) GetWebTextMatch() string {
	if o == nil || IsNil(o.WebTextMatch) {
		var ret string
		return ret
	}
	return *o.WebTextMatch
}

// GetWebTextMatchOk returns a tuple with the WebTextMatch field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebGetConfig4) GetWebTextMatchOk() (*string, bool) {
	if o == nil || IsNil(o.WebTextMatch) {
		return nil, false
	}
	return o.WebTextMatch, true
}

// IsSetWebTextMatch returns a boolean if a field has been set.
func (o *WebGetConfig4) IsSetWebTextMatch() bool {
	if o != nil && !IsNil(o.WebTextMatch) {
		return true
	}

	return false
}

// SetWebTextMatch gets a reference to the given string and assigns it to the WebTextMatch field.
func (o *WebGetConfig4) SetWebTextMatch(v string) {
	o.WebTextMatch = &v
}

func (o WebGetConfig4) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o WebGetConfig4) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["webMethod"] = o.WebMethod
	toSerialize["webUrl"] = o.WebUrl
	if !IsNil(o.IgnoreSSL) {
		toSerialize["ignoreSSL"] = o.IgnoreSSL
	}
	if !IsNil(o.CheckUser) {
		toSerialize["checkUser"] = o.CheckUser
	}
	if !IsNil(o.CheckPassword) {
		toSerialize["checkPassword"] = o.CheckPassword
	}
	if !IsNil(o.TextCheckOn) {
		toSerialize["textCheckOn"] = o.TextCheckOn
	}
	if !IsNil(o.WebTextMatch) {
		toSerialize["webTextMatch"] = o.WebTextMatch
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

type NullableWebGetConfig4 struct {
	value *WebGetConfig4
	isSet bool
}

func (v NullableWebGetConfig4) Get() *WebGetConfig4 {
	return v.value
}

func (v *NullableWebGetConfig4) Set(val *WebGetConfig4) {
	v.value = val
	v.isSet = true
}

func (v NullableWebGetConfig4) IsSet() bool {
	return v.isSet
}

func (v *NullableWebGetConfig4) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableWebGetConfig4(val *WebGetConfig4) *NullableWebGetConfig4 {
	return &NullableWebGetConfig4{value: val, isSet: true}
}

func (v NullableWebGetConfig4) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableWebGetConfig4) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

func (v NullableWebGetConfig4) UnmarshalMapstructure(data any) (any, error) {
	if err := mapstructDecode(data, &v.value); err != nil {
		return nil, err
	}
	v.isSet = true

	return v, nil
}

func (o *WebGetConfig4) UnmarshalJSON(data []byte) (err error) {
	return decode(data, &o)
}

// - model_simple.mustache
