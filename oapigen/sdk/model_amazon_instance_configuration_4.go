/*
Morpheus API

Morpheus is a powerful cloud management tool that provides provisioning, monitoring, logging, backups, and application deployment strategies.  This document describes the Morpheus API protocol and the available endpoints. Sections are organized in the same manner as they appear in the Morpheus UI.

API version: 9.0.0
Contact: dev@morpheusdata.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sdk

import (
	"encoding/json"
)

// checks if the AmazonInstanceConfiguration4 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AmazonInstanceConfiguration4{}

// AmazonInstanceConfiguration4 struct for AmazonInstanceConfiguration4
type AmazonInstanceConfiguration4 struct {
	// Skipping Agent installation will result in a lack of logging and guest operating system statistics. Automation scripts may also be adversely affected.
	NoAgent NullableBool `json:"noAgent,omitempty"`
	// Create user
	CreateUser NullableBool `json:"createUser,omitempty"`
	// Amazon Cloud Type
	IsEC2 *string `json:"isEC2,omitempty"`
	// Amazon Zone
	AvailabilityId *string `json:"availabilityId,omitempty"`
	// Security Group
	SecurityId *string `json:"securityId,omitempty"`
	// Public IP
	PublicIpType *string `json:"publicIpType,omitempty"`
	// IAM Profile
	InstanceProfile *string `json:"instanceProfile,omitempty"`
	// KMS Key ID
	KmsKeyId *string `json:"kmsKeyId,omitempty"`
	// Resource Pool ID
	ResourcePoolId       *string                `json:"resourcePoolId,omitempty"`
	AdditionalProperties map[string]interface{} `json:",remain"`
}

type _AmazonInstanceConfiguration4 AmazonInstanceConfiguration4

// NewAmazonInstanceConfiguration4 instantiates a new AmazonInstanceConfiguration4 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAmazonInstanceConfiguration4() *AmazonInstanceConfiguration4 {
	this := AmazonInstanceConfiguration4{}
	var noAgent bool = false
	this.NoAgent = *NewNullableBool(&noAgent)
	var createUser bool = false
	this.CreateUser = *NewNullableBool(&createUser)
	var isEC2 string = "false"
	this.IsEC2 = &isEC2
	return &this
}

// NewAmazonInstanceConfiguration4WithDefaults instantiates a new AmazonInstanceConfiguration4 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAmazonInstanceConfiguration4WithDefaults() *AmazonInstanceConfiguration4 {
	this := AmazonInstanceConfiguration4{}
	var noAgent bool = false
	this.NoAgent = *NewNullableBool(&noAgent)
	var createUser bool = false
	this.CreateUser = *NewNullableBool(&createUser)
	var isEC2 string = "false"
	this.IsEC2 = &isEC2
	return &this
}

// GetNoAgent returns the NoAgent field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AmazonInstanceConfiguration4) GetNoAgent() bool {
	if o == nil || IsNil(o.NoAgent.Get()) {
		var ret bool
		return ret
	}
	return *o.NoAgent.Get()
}

// GetNoAgentOk returns a tuple with the NoAgent field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AmazonInstanceConfiguration4) GetNoAgentOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return o.NoAgent.Get(), o.NoAgent.IsSet()
}

// IsSetNoAgent returns a boolean if a field has been set.
func (o *AmazonInstanceConfiguration4) IsSetNoAgent() bool {
	if o != nil && o.NoAgent.IsSet() {
		return true
	}

	return false
}

// SetNoAgent gets a reference to the given NullableBool and assigns it to the NoAgent field.
func (o *AmazonInstanceConfiguration4) SetNoAgent(v bool) {
	o.NoAgent.Set(&v)
}

// SetNoAgentNil sets the value for NoAgent to be an explicit nil
func (o *AmazonInstanceConfiguration4) SetNoAgentNil() {
	o.NoAgent.Set(nil)
}

// UnsetNoAgent ensures that no value is present for NoAgent, not even an explicit nil
func (o *AmazonInstanceConfiguration4) UnsetNoAgent() {
	o.NoAgent.Unset()
}

// GetCreateUser returns the CreateUser field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AmazonInstanceConfiguration4) GetCreateUser() bool {
	if o == nil || IsNil(o.CreateUser.Get()) {
		var ret bool
		return ret
	}
	return *o.CreateUser.Get()
}

// GetCreateUserOk returns a tuple with the CreateUser field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AmazonInstanceConfiguration4) GetCreateUserOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return o.CreateUser.Get(), o.CreateUser.IsSet()
}

// IsSetCreateUser returns a boolean if a field has been set.
func (o *AmazonInstanceConfiguration4) IsSetCreateUser() bool {
	if o != nil && o.CreateUser.IsSet() {
		return true
	}

	return false
}

// SetCreateUser gets a reference to the given NullableBool and assigns it to the CreateUser field.
func (o *AmazonInstanceConfiguration4) SetCreateUser(v bool) {
	o.CreateUser.Set(&v)
}

// SetCreateUserNil sets the value for CreateUser to be an explicit nil
func (o *AmazonInstanceConfiguration4) SetCreateUserNil() {
	o.CreateUser.Set(nil)
}

// UnsetCreateUser ensures that no value is present for CreateUser, not even an explicit nil
func (o *AmazonInstanceConfiguration4) UnsetCreateUser() {
	o.CreateUser.Unset()
}

// GetIsEC2 returns the IsEC2 field value if set, zero value otherwise.
func (o *AmazonInstanceConfiguration4) GetIsEC2() string {
	if o == nil || IsNil(o.IsEC2) {
		var ret string
		return ret
	}
	return *o.IsEC2
}

// GetIsEC2Ok returns a tuple with the IsEC2 field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AmazonInstanceConfiguration4) GetIsEC2Ok() (*string, bool) {
	if o == nil || IsNil(o.IsEC2) {
		return nil, false
	}
	return o.IsEC2, true
}

// IsSetIsEC2 returns a boolean if a field has been set.
func (o *AmazonInstanceConfiguration4) IsSetIsEC2() bool {
	if o != nil && !IsNil(o.IsEC2) {
		return true
	}

	return false
}

// SetIsEC2 gets a reference to the given string and assigns it to the IsEC2 field.
func (o *AmazonInstanceConfiguration4) SetIsEC2(v string) {
	o.IsEC2 = &v
}

// GetAvailabilityId returns the AvailabilityId field value if set, zero value otherwise.
func (o *AmazonInstanceConfiguration4) GetAvailabilityId() string {
	if o == nil || IsNil(o.AvailabilityId) {
		var ret string
		return ret
	}
	return *o.AvailabilityId
}

// GetAvailabilityIdOk returns a tuple with the AvailabilityId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AmazonInstanceConfiguration4) GetAvailabilityIdOk() (*string, bool) {
	if o == nil || IsNil(o.AvailabilityId) {
		return nil, false
	}
	return o.AvailabilityId, true
}

// IsSetAvailabilityId returns a boolean if a field has been set.
func (o *AmazonInstanceConfiguration4) IsSetAvailabilityId() bool {
	if o != nil && !IsNil(o.AvailabilityId) {
		return true
	}

	return false
}

// SetAvailabilityId gets a reference to the given string and assigns it to the AvailabilityId field.
func (o *AmazonInstanceConfiguration4) SetAvailabilityId(v string) {
	o.AvailabilityId = &v
}

// GetSecurityId returns the SecurityId field value if set, zero value otherwise.
func (o *AmazonInstanceConfiguration4) GetSecurityId() string {
	if o == nil || IsNil(o.SecurityId) {
		var ret string
		return ret
	}
	return *o.SecurityId
}

// GetSecurityIdOk returns a tuple with the SecurityId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AmazonInstanceConfiguration4) GetSecurityIdOk() (*string, bool) {
	if o == nil || IsNil(o.SecurityId) {
		return nil, false
	}
	return o.SecurityId, true
}

// IsSetSecurityId returns a boolean if a field has been set.
func (o *AmazonInstanceConfiguration4) IsSetSecurityId() bool {
	if o != nil && !IsNil(o.SecurityId) {
		return true
	}

	return false
}

// SetSecurityId gets a reference to the given string and assigns it to the SecurityId field.
func (o *AmazonInstanceConfiguration4) SetSecurityId(v string) {
	o.SecurityId = &v
}

// GetPublicIpType returns the PublicIpType field value if set, zero value otherwise.
func (o *AmazonInstanceConfiguration4) GetPublicIpType() string {
	if o == nil || IsNil(o.PublicIpType) {
		var ret string
		return ret
	}
	return *o.PublicIpType
}

// GetPublicIpTypeOk returns a tuple with the PublicIpType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AmazonInstanceConfiguration4) GetPublicIpTypeOk() (*string, bool) {
	if o == nil || IsNil(o.PublicIpType) {
		return nil, false
	}
	return o.PublicIpType, true
}

// IsSetPublicIpType returns a boolean if a field has been set.
func (o *AmazonInstanceConfiguration4) IsSetPublicIpType() bool {
	if o != nil && !IsNil(o.PublicIpType) {
		return true
	}

	return false
}

// SetPublicIpType gets a reference to the given string and assigns it to the PublicIpType field.
func (o *AmazonInstanceConfiguration4) SetPublicIpType(v string) {
	o.PublicIpType = &v
}

// GetInstanceProfile returns the InstanceProfile field value if set, zero value otherwise.
func (o *AmazonInstanceConfiguration4) GetInstanceProfile() string {
	if o == nil || IsNil(o.InstanceProfile) {
		var ret string
		return ret
	}
	return *o.InstanceProfile
}

// GetInstanceProfileOk returns a tuple with the InstanceProfile field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AmazonInstanceConfiguration4) GetInstanceProfileOk() (*string, bool) {
	if o == nil || IsNil(o.InstanceProfile) {
		return nil, false
	}
	return o.InstanceProfile, true
}

// IsSetInstanceProfile returns a boolean if a field has been set.
func (o *AmazonInstanceConfiguration4) IsSetInstanceProfile() bool {
	if o != nil && !IsNil(o.InstanceProfile) {
		return true
	}

	return false
}

// SetInstanceProfile gets a reference to the given string and assigns it to the InstanceProfile field.
func (o *AmazonInstanceConfiguration4) SetInstanceProfile(v string) {
	o.InstanceProfile = &v
}

// GetKmsKeyId returns the KmsKeyId field value if set, zero value otherwise.
func (o *AmazonInstanceConfiguration4) GetKmsKeyId() string {
	if o == nil || IsNil(o.KmsKeyId) {
		var ret string
		return ret
	}
	return *o.KmsKeyId
}

// GetKmsKeyIdOk returns a tuple with the KmsKeyId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AmazonInstanceConfiguration4) GetKmsKeyIdOk() (*string, bool) {
	if o == nil || IsNil(o.KmsKeyId) {
		return nil, false
	}
	return o.KmsKeyId, true
}

// IsSetKmsKeyId returns a boolean if a field has been set.
func (o *AmazonInstanceConfiguration4) IsSetKmsKeyId() bool {
	if o != nil && !IsNil(o.KmsKeyId) {
		return true
	}

	return false
}

// SetKmsKeyId gets a reference to the given string and assigns it to the KmsKeyId field.
func (o *AmazonInstanceConfiguration4) SetKmsKeyId(v string) {
	o.KmsKeyId = &v
}

// GetResourcePoolId returns the ResourcePoolId field value if set, zero value otherwise.
func (o *AmazonInstanceConfiguration4) GetResourcePoolId() string {
	if o == nil || IsNil(o.ResourcePoolId) {
		var ret string
		return ret
	}
	return *o.ResourcePoolId
}

// GetResourcePoolIdOk returns a tuple with the ResourcePoolId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AmazonInstanceConfiguration4) GetResourcePoolIdOk() (*string, bool) {
	if o == nil || IsNil(o.ResourcePoolId) {
		return nil, false
	}
	return o.ResourcePoolId, true
}

// IsSetResourcePoolId returns a boolean if a field has been set.
func (o *AmazonInstanceConfiguration4) IsSetResourcePoolId() bool {
	if o != nil && !IsNil(o.ResourcePoolId) {
		return true
	}

	return false
}

// SetResourcePoolId gets a reference to the given string and assigns it to the ResourcePoolId field.
func (o *AmazonInstanceConfiguration4) SetResourcePoolId(v string) {
	o.ResourcePoolId = &v
}

func (o AmazonInstanceConfiguration4) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AmazonInstanceConfiguration4) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.NoAgent.IsSet() {
		toSerialize["noAgent"] = o.NoAgent.Get()
	}
	if o.CreateUser.IsSet() {
		toSerialize["createUser"] = o.CreateUser.Get()
	}
	if !IsNil(o.IsEC2) {
		toSerialize["isEC2"] = o.IsEC2
	}
	if !IsNil(o.AvailabilityId) {
		toSerialize["availabilityId"] = o.AvailabilityId
	}
	if !IsNil(o.SecurityId) {
		toSerialize["securityId"] = o.SecurityId
	}
	if !IsNil(o.PublicIpType) {
		toSerialize["publicIpType"] = o.PublicIpType
	}
	if !IsNil(o.InstanceProfile) {
		toSerialize["instanceProfile"] = o.InstanceProfile
	}
	if !IsNil(o.KmsKeyId) {
		toSerialize["kmsKeyId"] = o.KmsKeyId
	}
	if !IsNil(o.ResourcePoolId) {
		toSerialize["resourcePoolId"] = o.ResourcePoolId
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

type NullableAmazonInstanceConfiguration4 struct {
	value *AmazonInstanceConfiguration4
	isSet bool
}

func (v NullableAmazonInstanceConfiguration4) Get() *AmazonInstanceConfiguration4 {
	return v.value
}

func (v *NullableAmazonInstanceConfiguration4) Set(val *AmazonInstanceConfiguration4) {
	v.value = val
	v.isSet = true
}

func (v NullableAmazonInstanceConfiguration4) IsSet() bool {
	return v.isSet
}

func (v *NullableAmazonInstanceConfiguration4) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAmazonInstanceConfiguration4(val *AmazonInstanceConfiguration4) *NullableAmazonInstanceConfiguration4 {
	return &NullableAmazonInstanceConfiguration4{value: val, isSet: true}
}

func (v NullableAmazonInstanceConfiguration4) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAmazonInstanceConfiguration4) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

func (v NullableAmazonInstanceConfiguration4) UnmarshalMapstructure(data any) (any, error) {
	if err := mapstructDecode(data, &v.value); err != nil {
		return nil, err
	}
	v.isSet = true

	return v, nil
}

func (o *AmazonInstanceConfiguration4) UnmarshalJSON(data []byte) (err error) {
	return decode(data, &o)
}

// - model_simple.mustache
