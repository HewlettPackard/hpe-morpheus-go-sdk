...
```go
obj := &ListOsTypes200ResponseAllOfOsTypesInner{
    // Set fields directly
}
```
...
Use the Nullable wrapper methods:
- `obj.Images.IsSet()` — check if set
- `obj.Images.Get()` — get the inner value (returns pointer)
- `obj.Images.Set(&val)` — set the value
- `obj.Images.Unset()` — clear the value
