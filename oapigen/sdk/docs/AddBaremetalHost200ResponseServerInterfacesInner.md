...
```go
obj := &AddBaremetalHost200ResponseServerInterfacesInner{
    // Set fields directly
}
```
...
### PublicIpAddress (Nullable)

Use the Nullable wrapper methods:
- `obj.PublicIpAddress.IsSet()` — check if set
- `obj.PublicIpAddress.Get()` — get the inner value (returns pointer)
- `obj.PublicIpAddress.Set(&val)` — set the value
- `obj.PublicIpAddress.Unset()` — clear the value
