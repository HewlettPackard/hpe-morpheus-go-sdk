...
```go
obj := &ListIncidents200ResponseAllOfIssuesInner{
    // Set fields directly
}
```
...
Use the Nullable wrapper methods:
- `obj.CheckGroup.IsSet()` — check if set
- `obj.CheckGroup.Get()` — get the inner value (returns pointer)
- `obj.CheckGroup.Set(&val)` — set the value
- `obj.CheckGroup.Unset()` — clear the value
