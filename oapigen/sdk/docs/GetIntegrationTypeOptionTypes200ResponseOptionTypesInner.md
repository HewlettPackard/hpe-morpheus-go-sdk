...
```go
obj := &GetIntegrationTypeOptionTypes200ResponseOptionTypesInner{
    // Set fields directly
}
```
...
Use the Nullable wrapper methods:
- `obj.ExportMeta.IsSet()` — check if set
- `obj.ExportMeta.Get()` — get the inner value (returns pointer)
- `obj.ExportMeta.Set(&val)` — set the value
- `obj.ExportMeta.Unset()` — clear the value
...
Use the Nullable wrapper methods:
- `obj.NoBlank.IsSet()` — check if set
- `obj.NoBlank.Get()` — get the inner value (returns pointer)
- `obj.NoBlank.Set(&val)` — set the value
- `obj.NoBlank.Unset()` — clear the value
