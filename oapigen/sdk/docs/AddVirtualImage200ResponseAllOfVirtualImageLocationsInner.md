...
```go
obj := &AddVirtualImage200ResponseAllOfVirtualImageLocationsInner{
    // Set fields directly
}
```
...
### ExternalDiskId (Nullable)

Use the Nullable wrapper methods:
- `obj.ExternalDiskId.IsSet()` — check if set
- `obj.ExternalDiskId.Get()` — get the inner value (returns pointer)
- `obj.ExternalDiskId.Set(&val)` — set the value
- `obj.ExternalDiskId.Unset()` — clear the value
### RemotePath (Nullable)
...
### ImageRegion (Nullable)

Use the Nullable wrapper methods:
- `obj.ImageRegion.IsSet()` — check if set
- `obj.ImageRegion.Get()` — get the inner value (returns pointer)
- `obj.ImageRegion.Set(&val)` — set the value
- `obj.ImageRegion.Unset()` — clear the value
