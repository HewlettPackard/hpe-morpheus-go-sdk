...
```go
obj := &VRealizeOrchestratorTaskConfig1{
    // Set fields directly
}
```
...
Use the Nullable wrapper methods:
- `obj.VroBody.IsSet()` — check if set
- `obj.VroBody.Get()` — get the inner value (returns pointer)
- `obj.VroBody.Set(&val)` — set the value
- `obj.VroBody.Unset()` — clear the value
### Password (Nullable)
