...
```go
obj := &ListHistory200ResponseAllOfProcessesInner{
    // Set fields directly
}
```
...
Use the Nullable wrapper methods:
- `obj.ResultId.IsSet()` — check if set
- `obj.ResultId.Get()` — get the inner value (returns pointer)
- `obj.ResultId.Set(&val)` — set the value
- `obj.ResultId.Unset()` — clear the value
