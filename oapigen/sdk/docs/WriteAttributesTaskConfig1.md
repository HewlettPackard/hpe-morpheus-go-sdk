...
```go
obj := &WriteAttributesTaskConfig1{
    // Set fields directly
}
```
...
### WriteAttributesAttributes (Nullable)

Use the Nullable wrapper methods:
- `obj.WriteAttributesAttributes.IsSet()` — check if set
- `obj.WriteAttributesAttributes.Get()` — get the inner value (returns pointer)
- `obj.WriteAttributesAttributes.Set(&val)` — set the value
- `obj.WriteAttributesAttributes.Unset()` — clear the value
### Username (Nullable)
