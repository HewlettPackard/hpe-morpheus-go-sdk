/*
Morpheus API

Morpheus is a powerful cloud management tool that provides provisioning, monitoring, logging, backups, and application deployment strategies.  This document describes the Morpheus API protocol and the available endpoints. Sections are organized in the same manner as they appear in the Morpheus UI.

API version: 9.0.0
Contact: dev@morpheusdata.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sdk

import (
	"encoding/json"
	"time"
)

// checks if the GetInstanceDeploys200ResponseAllOfAppDeploysInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetInstanceDeploys200ResponseAllOfAppDeploysInner{}

// GetInstanceDeploys200ResponseAllOfAppDeploysInner struct for GetInstanceDeploys200ResponseAllOfAppDeploysInner
type GetInstanceDeploys200ResponseAllOfAppDeploysInner struct {
	Id                   *int64                                                              `json:"id,omitempty"`
	InstanceId           *int64                                                              `json:"instanceId,omitempty"`
	Instance             *GetInstanceDeploys200ResponseAllOfAppDeploysInnerInstance          `json:"instance,omitempty"`
	Deployment           *GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeployment        `json:"deployment,omitempty"`
	DeploymentVersionId  *int64                                                              `json:"deploymentVersionId,omitempty"`
	DeploymentVersion    *GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeploymentVersion `json:"deploymentVersion,omitempty"`
	Config               map[string]interface{}                                              `json:"config,omitempty"`
	Status               *string                                                             `json:"status,omitempty"`
	DeployDate           *time.Time                                                          `json:"deployDate,omitempty"`
	DateCreated          *time.Time                                                          `json:"dateCreated,omitempty"`
	LastUpdated          *time.Time                                                          `json:"lastUpdated,omitempty"`
	AdditionalProperties map[string]interface{}                                              `json:",remain"`
}

type _GetInstanceDeploys200ResponseAllOfAppDeploysInner GetInstanceDeploys200ResponseAllOfAppDeploysInner

// NewGetInstanceDeploys200ResponseAllOfAppDeploysInner instantiates a new GetInstanceDeploys200ResponseAllOfAppDeploysInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetInstanceDeploys200ResponseAllOfAppDeploysInner() *GetInstanceDeploys200ResponseAllOfAppDeploysInner {
	this := GetInstanceDeploys200ResponseAllOfAppDeploysInner{}
	return &this
}

// NewGetInstanceDeploys200ResponseAllOfAppDeploysInnerWithDefaults instantiates a new GetInstanceDeploys200ResponseAllOfAppDeploysInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetInstanceDeploys200ResponseAllOfAppDeploysInnerWithDefaults() *GetInstanceDeploys200ResponseAllOfAppDeploysInner {
	this := GetInstanceDeploys200ResponseAllOfAppDeploysInner{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetId() int64 {
	if o == nil || IsNil(o.Id) {
		var ret int64
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetIdOk() (*int64, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// IsSetId returns a boolean if a field has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSetId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given int64 and assigns it to the Id field.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) SetId(v int64) {
	o.Id = &v
}

// GetInstanceId returns the InstanceId field value if set, zero value otherwise.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetInstanceId() int64 {
	if o == nil || IsNil(o.InstanceId) {
		var ret int64
		return ret
	}
	return *o.InstanceId
}

// GetInstanceIdOk returns a tuple with the InstanceId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetInstanceIdOk() (*int64, bool) {
	if o == nil || IsNil(o.InstanceId) {
		return nil, false
	}
	return o.InstanceId, true
}

// IsSetInstanceId returns a boolean if a field has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSetInstanceId() bool {
	if o != nil && !IsNil(o.InstanceId) {
		return true
	}

	return false
}

// SetInstanceId gets a reference to the given int64 and assigns it to the InstanceId field.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) SetInstanceId(v int64) {
	o.InstanceId = &v
}

// GetInstance returns the Instance field value if set, zero value otherwise.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetInstance() GetInstanceDeploys200ResponseAllOfAppDeploysInnerInstance {
	if o == nil || IsNil(o.Instance) {
		var ret GetInstanceDeploys200ResponseAllOfAppDeploysInnerInstance
		return ret
	}
	return *o.Instance
}

// GetInstanceOk returns a tuple with the Instance field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetInstanceOk() (*GetInstanceDeploys200ResponseAllOfAppDeploysInnerInstance, bool) {
	if o == nil || IsNil(o.Instance) {
		return nil, false
	}
	return o.Instance, true
}

// IsSetInstance returns a boolean if a field has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSetInstance() bool {
	if o != nil && !IsNil(o.Instance) {
		return true
	}

	return false
}

// SetInstance gets a reference to the given GetInstanceDeploys200ResponseAllOfAppDeploysInnerInstance and assigns it to the Instance field.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) SetInstance(v GetInstanceDeploys200ResponseAllOfAppDeploysInnerInstance) {
	o.Instance = &v
}

// GetDeployment returns the Deployment field value if set, zero value otherwise.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetDeployment() GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeployment {
	if o == nil || IsNil(o.Deployment) {
		var ret GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeployment
		return ret
	}
	return *o.Deployment
}

// GetDeploymentOk returns a tuple with the Deployment field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetDeploymentOk() (*GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeployment, bool) {
	if o == nil || IsNil(o.Deployment) {
		return nil, false
	}
	return o.Deployment, true
}

// IsSetDeployment returns a boolean if a field has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSetDeployment() bool {
	if o != nil && !IsNil(o.Deployment) {
		return true
	}

	return false
}

// SetDeployment gets a reference to the given GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeployment and assigns it to the Deployment field.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) SetDeployment(v GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeployment) {
	o.Deployment = &v
}

// GetDeploymentVersionId returns the DeploymentVersionId field value if set, zero value otherwise.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetDeploymentVersionId() int64 {
	if o == nil || IsNil(o.DeploymentVersionId) {
		var ret int64
		return ret
	}
	return *o.DeploymentVersionId
}

// GetDeploymentVersionIdOk returns a tuple with the DeploymentVersionId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetDeploymentVersionIdOk() (*int64, bool) {
	if o == nil || IsNil(o.DeploymentVersionId) {
		return nil, false
	}
	return o.DeploymentVersionId, true
}

// IsSetDeploymentVersionId returns a boolean if a field has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSetDeploymentVersionId() bool {
	if o != nil && !IsNil(o.DeploymentVersionId) {
		return true
	}

	return false
}

// SetDeploymentVersionId gets a reference to the given int64 and assigns it to the DeploymentVersionId field.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) SetDeploymentVersionId(v int64) {
	o.DeploymentVersionId = &v
}

// GetDeploymentVersion returns the DeploymentVersion field value if set, zero value otherwise.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetDeploymentVersion() GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeploymentVersion {
	if o == nil || IsNil(o.DeploymentVersion) {
		var ret GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeploymentVersion
		return ret
	}
	return *o.DeploymentVersion
}

// GetDeploymentVersionOk returns a tuple with the DeploymentVersion field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetDeploymentVersionOk() (*GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeploymentVersion, bool) {
	if o == nil || IsNil(o.DeploymentVersion) {
		return nil, false
	}
	return o.DeploymentVersion, true
}

// IsSetDeploymentVersion returns a boolean if a field has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSetDeploymentVersion() bool {
	if o != nil && !IsNil(o.DeploymentVersion) {
		return true
	}

	return false
}

// SetDeploymentVersion gets a reference to the given GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeploymentVersion and assigns it to the DeploymentVersion field.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) SetDeploymentVersion(v GetInstanceDeploys200ResponseAllOfAppDeploysInnerDeploymentVersion) {
	o.DeploymentVersion = &v
}

// GetConfig returns the Config field value if set, zero value otherwise.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetConfig() map[string]interface{} {
	if o == nil || IsNil(o.Config) {
		var ret map[string]interface{}
		return ret
	}
	return o.Config
}

// GetConfigOk returns a tuple with the Config field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetConfigOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.Config) {
		return map[string]interface{}{}, false
	}
	return o.Config, true
}

// IsSetConfig returns a boolean if a field has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSetConfig() bool {
	if o != nil && !IsNil(o.Config) {
		return true
	}

	return false
}

// SetConfig gets a reference to the given map[string]interface{} and assigns it to the Config field.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) SetConfig(v map[string]interface{}) {
	o.Config = v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetStatus() string {
	if o == nil || IsNil(o.Status) {
		var ret string
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetStatusOk() (*string, bool) {
	if o == nil || IsNil(o.Status) {
		return nil, false
	}
	return o.Status, true
}

// IsSetStatus returns a boolean if a field has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSetStatus() bool {
	if o != nil && !IsNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given string and assigns it to the Status field.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) SetStatus(v string) {
	o.Status = &v
}

// GetDeployDate returns the DeployDate field value if set, zero value otherwise.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetDeployDate() time.Time {
	if o == nil || IsNil(o.DeployDate) {
		var ret time.Time
		return ret
	}
	return *o.DeployDate
}

// GetDeployDateOk returns a tuple with the DeployDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetDeployDateOk() (*time.Time, bool) {
	if o == nil || IsNil(o.DeployDate) {
		return nil, false
	}
	return o.DeployDate, true
}

// IsSetDeployDate returns a boolean if a field has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSetDeployDate() bool {
	if o != nil && !IsNil(o.DeployDate) {
		return true
	}

	return false
}

// SetDeployDate gets a reference to the given time.Time and assigns it to the DeployDate field.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) SetDeployDate(v time.Time) {
	o.DeployDate = &v
}

// GetDateCreated returns the DateCreated field value if set, zero value otherwise.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetDateCreated() time.Time {
	if o == nil || IsNil(o.DateCreated) {
		var ret time.Time
		return ret
	}
	return *o.DateCreated
}

// GetDateCreatedOk returns a tuple with the DateCreated field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetDateCreatedOk() (*time.Time, bool) {
	if o == nil || IsNil(o.DateCreated) {
		return nil, false
	}
	return o.DateCreated, true
}

// IsSetDateCreated returns a boolean if a field has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSetDateCreated() bool {
	if o != nil && !IsNil(o.DateCreated) {
		return true
	}

	return false
}

// SetDateCreated gets a reference to the given time.Time and assigns it to the DateCreated field.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) SetDateCreated(v time.Time) {
	o.DateCreated = &v
}

// GetLastUpdated returns the LastUpdated field value if set, zero value otherwise.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetLastUpdated() time.Time {
	if o == nil || IsNil(o.LastUpdated) {
		var ret time.Time
		return ret
	}
	return *o.LastUpdated
}

// GetLastUpdatedOk returns a tuple with the LastUpdated field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) GetLastUpdatedOk() (*time.Time, bool) {
	if o == nil || IsNil(o.LastUpdated) {
		return nil, false
	}
	return o.LastUpdated, true
}

// IsSetLastUpdated returns a boolean if a field has been set.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSetLastUpdated() bool {
	if o != nil && !IsNil(o.LastUpdated) {
		return true
	}

	return false
}

// SetLastUpdated gets a reference to the given time.Time and assigns it to the LastUpdated field.
func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) SetLastUpdated(v time.Time) {
	o.LastUpdated = &v
}

func (o GetInstanceDeploys200ResponseAllOfAppDeploysInner) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetInstanceDeploys200ResponseAllOfAppDeploysInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.InstanceId) {
		toSerialize["instanceId"] = o.InstanceId
	}
	if !IsNil(o.Instance) {
		toSerialize["instance"] = o.Instance
	}
	if !IsNil(o.Deployment) {
		toSerialize["deployment"] = o.Deployment
	}
	if !IsNil(o.DeploymentVersionId) {
		toSerialize["deploymentVersionId"] = o.DeploymentVersionId
	}
	if !IsNil(o.DeploymentVersion) {
		toSerialize["deploymentVersion"] = o.DeploymentVersion
	}
	if !IsNil(o.Config) {
		toSerialize["config"] = o.Config
	}
	if !IsNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	if !IsNil(o.DeployDate) {
		toSerialize["deployDate"] = o.DeployDate
	}
	if !IsNil(o.DateCreated) {
		toSerialize["dateCreated"] = o.DateCreated
	}
	if !IsNil(o.LastUpdated) {
		toSerialize["lastUpdated"] = o.LastUpdated
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

type NullableGetInstanceDeploys200ResponseAllOfAppDeploysInner struct {
	value *GetInstanceDeploys200ResponseAllOfAppDeploysInner
	isSet bool
}

func (v NullableGetInstanceDeploys200ResponseAllOfAppDeploysInner) Get() *GetInstanceDeploys200ResponseAllOfAppDeploysInner {
	return v.value
}

func (v *NullableGetInstanceDeploys200ResponseAllOfAppDeploysInner) Set(val *GetInstanceDeploys200ResponseAllOfAppDeploysInner) {
	v.value = val
	v.isSet = true
}

func (v NullableGetInstanceDeploys200ResponseAllOfAppDeploysInner) IsSet() bool {
	return v.isSet
}

func (v *NullableGetInstanceDeploys200ResponseAllOfAppDeploysInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetInstanceDeploys200ResponseAllOfAppDeploysInner(val *GetInstanceDeploys200ResponseAllOfAppDeploysInner) *NullableGetInstanceDeploys200ResponseAllOfAppDeploysInner {
	return &NullableGetInstanceDeploys200ResponseAllOfAppDeploysInner{value: val, isSet: true}
}

func (v NullableGetInstanceDeploys200ResponseAllOfAppDeploysInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetInstanceDeploys200ResponseAllOfAppDeploysInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

func (v NullableGetInstanceDeploys200ResponseAllOfAppDeploysInner) UnmarshalMapstructure(data any) (any, error) {
	if err := mapstructDecode(data, &v.value); err != nil {
		return nil, err
	}
	v.isSet = true

	return v, nil
}

func (o *GetInstanceDeploys200ResponseAllOfAppDeploysInner) UnmarshalJSON(data []byte) (err error) {
	return decode(data, &o)
}

// - model_simple.mustache
