/*
Morpheus API

Morpheus is a powerful cloud management tool that provides provisioning, monitoring, logging, backups, and application deployment strategies.  This document describes the Morpheus API protocol and the available endpoints. Sections are organized in the same manner as they appear in the Morpheus UI.

API version: 9.0.0
Contact: dev@morpheusdata.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sdk

import (
	"encoding/json"
)

// checks if the BackupServerHost1 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &BackupServerHost1{}

// BackupServerHost1 struct for BackupServerHost1
type BackupServerHost1 struct {
	LocationType string `json:"locationType"`
	// A name for the backup
	Name string `json:"name"`
	// The ID of the server to backup
	ServerId int64 `json:"serverId"`
	// The backup type code for server backups
	BackupType string `json:"backupType"`
	// The file or directory path on the target host to back up.
	TargetPath *string `json:"targetPath,omitempty"`
	// SSH username for connecting to the server.
	SshUsername *string `json:"sshUsername,omitempty"`
	// SSH password for connecting to the server.
	SshPassword *string `json:"sshPassword,omitempty"`
	// The ID of the storage bucket to store the backup in.
	Target *int64 `json:"target,omitempty"`
	// Create a new backup job, clone an existing job or add the new backup to an existing job. If not specified, defaults to `new`.
	JobAction string `json:"jobAction"`
	// The ID of the job to clone or add to. Only applies to jobAction `clone` and `addTo`.
	JobId *int64 `json:"jobId,omitempty"`
	// Name for new job. Only applies to jobAction `new` and `clone`. Defaults to the backup name if not specified.
	JobName *string `json:"jobName,omitempty"`
	// The ID of the execute schedule for new job. See Execute Schedules. Only applies to jobAction `new` and `clone`.
	JobSchedule *int64 `json:"jobSchedule,omitempty"`
	// Retention Count for new job. By default the backup settings value will be used. Only applies to jobAction `new` and `clone`.
	RetentionCount       *int64                 `json:"retentionCount,omitempty"`
	AdditionalProperties map[string]interface{} `json:",remain"`
}

type _BackupServerHost1 BackupServerHost1

func (o BackupServerHost1) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o BackupServerHost1) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["locationType"] = o.LocationType
	toSerialize["name"] = o.Name
	toSerialize["serverId"] = o.ServerId
	toSerialize["backupType"] = o.BackupType
	if !IsNil(o.TargetPath) {
		toSerialize["targetPath"] = o.TargetPath
	}
	if !IsNil(o.SshUsername) {
		toSerialize["sshUsername"] = o.SshUsername
	}
	if !IsNil(o.SshPassword) {
		toSerialize["sshPassword"] = o.SshPassword
	}
	if !IsNil(o.Target) {
		toSerialize["target"] = o.Target
	}
	toSerialize["jobAction"] = o.JobAction
	if !IsNil(o.JobId) {
		toSerialize["jobId"] = o.JobId
	}
	if !IsNil(o.JobName) {
		toSerialize["jobName"] = o.JobName
	}
	if !IsNil(o.JobSchedule) {
		toSerialize["jobSchedule"] = o.JobSchedule
	}
	if !IsNil(o.RetentionCount) {
		toSerialize["retentionCount"] = o.RetentionCount
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

type NullableBackupServerHost1 struct {
	value *BackupServerHost1
	isSet bool
}

func (v NullableBackupServerHost1) Get() *BackupServerHost1 {
	return v.value
}

func (v *NullableBackupServerHost1) Set(val *BackupServerHost1) {
	v.value = val
	v.isSet = true
}

func (v NullableBackupServerHost1) IsSet() bool {
	return v.isSet
}

func (v *NullableBackupServerHost1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBackupServerHost1(val *BackupServerHost1) *NullableBackupServerHost1 {
	return &NullableBackupServerHost1{value: val, isSet: true}
}

func (v NullableBackupServerHost1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBackupServerHost1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

func (v NullableBackupServerHost1) UnmarshalMapstructure(data any) (any, error) {
	if err := mapstructDecode(data, &v.value); err != nil {
		return nil, err
	}
	v.isSet = true

	return v, nil
}

func (o *BackupServerHost1) UnmarshalJSON(data []byte) (err error) {
	return decode(data, &o)
}

// - model_simple.mustache
