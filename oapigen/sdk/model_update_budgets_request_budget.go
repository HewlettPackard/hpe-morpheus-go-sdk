/*
Morpheus API

Morpheus is a powerful cloud management tool that provides provisioning, monitoring, logging, backups, and application deployment strategies.  This document describes the Morpheus API protocol and the available endpoints. Sections are organized in the same manner as they appear in the Morpheus UI.

API version: 8.1.1
Contact: dev@morpheusdata.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sdk

import (
	"encoding/json"
	"time"
)

// checks if the UpdateBudgetsRequestBudget type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UpdateBudgetsRequestBudget{}

// UpdateBudgetsRequestBudget struct for UpdateBudgetsRequestBudget
type UpdateBudgetsRequestBudget struct {
	Name        string     `json:"name"`
	Description *string    `json:"description,omitempty"`
	Scope       *string    `json:"scope,omitempty"`
	Period      *string    `json:"period,omitempty"`
	Year        *int64     `json:"year,omitempty"`
	StartDate   *time.Time `json:"startDate,omitempty"`
	EndDate     *time.Time `json:"endDate,omitempty"`
	Interval    *string    `json:"interval,omitempty"`
	// The Tenant ID to scope to, for use with ``scope``=tenant
	ScopeTenantId *int64 `json:"scopeTenantId,omitempty"`
	// The Tenant ID to scope to, for use with ``scope``=group
	ScopeGroupId *int64 `json:"scopeGroupId,omitempty"`
	// The Tenant ID to scope to, for use with ``scope``=cloud
	ScopeCloudId *int64 `json:"scopeCloudId,omitempty"`
	// The Tenant ID to scope to, for use with ``scope``=user
	ScopeUserId          *int64                                  `json:"scopeUserId,omitempty"`
	Costs                []int64                                 `json:"costs,omitempty"`
	Enabled              *bool                                   `json:"enabled,omitempty"`
	ForecastType         *UpdateBudgetsRequestBudgetForecastType `json:"forecastType,omitempty"`
	AdditionalProperties map[string]interface{}                  `json:",remain"`
}

type _UpdateBudgetsRequestBudget UpdateBudgetsRequestBudget

// NewUpdateBudgetsRequestBudget instantiates a new UpdateBudgetsRequestBudget object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUpdateBudgetsRequestBudget(name string) *UpdateBudgetsRequestBudget {
	this := UpdateBudgetsRequestBudget{}
	this.Name = name
	var scope string = "account"
	this.Scope = &scope
	var period string = "year"
	this.Period = &period
	var interval string = "year"
	this.Interval = &interval
	var enabled bool = true
	this.Enabled = &enabled
	return &this
}

// NewUpdateBudgetsRequestBudgetWithDefaults instantiates a new UpdateBudgetsRequestBudget object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUpdateBudgetsRequestBudgetWithDefaults() *UpdateBudgetsRequestBudget {
	this := UpdateBudgetsRequestBudget{}
	var scope string = "account"
	this.Scope = &scope
	var period string = "year"
	this.Period = &period
	var interval string = "year"
	this.Interval = &interval
	var enabled bool = true
	this.Enabled = &enabled
	return &this
}

// GetName returns the Name field value
func (o *UpdateBudgetsRequestBudget) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *UpdateBudgetsRequestBudget) SetName(v string) {
	o.Name = v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// IsSetDescription returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *UpdateBudgetsRequestBudget) SetDescription(v string) {
	o.Description = &v
}

// GetScope returns the Scope field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetScope() string {
	if o == nil || IsNil(o.Scope) {
		var ret string
		return ret
	}
	return *o.Scope
}

// GetScopeOk returns a tuple with the Scope field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetScopeOk() (*string, bool) {
	if o == nil || IsNil(o.Scope) {
		return nil, false
	}
	return o.Scope, true
}

// IsSetScope returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetScope() bool {
	if o != nil && !IsNil(o.Scope) {
		return true
	}

	return false
}

// SetScope gets a reference to the given string and assigns it to the Scope field.
func (o *UpdateBudgetsRequestBudget) SetScope(v string) {
	o.Scope = &v
}

// GetPeriod returns the Period field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetPeriod() string {
	if o == nil || IsNil(o.Period) {
		var ret string
		return ret
	}
	return *o.Period
}

// GetPeriodOk returns a tuple with the Period field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetPeriodOk() (*string, bool) {
	if o == nil || IsNil(o.Period) {
		return nil, false
	}
	return o.Period, true
}

// IsSetPeriod returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetPeriod() bool {
	if o != nil && !IsNil(o.Period) {
		return true
	}

	return false
}

// SetPeriod gets a reference to the given string and assigns it to the Period field.
func (o *UpdateBudgetsRequestBudget) SetPeriod(v string) {
	o.Period = &v
}

// GetYear returns the Year field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetYear() int64 {
	if o == nil || IsNil(o.Year) {
		var ret int64
		return ret
	}
	return *o.Year
}

// GetYearOk returns a tuple with the Year field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetYearOk() (*int64, bool) {
	if o == nil || IsNil(o.Year) {
		return nil, false
	}
	return o.Year, true
}

// IsSetYear returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetYear() bool {
	if o != nil && !IsNil(o.Year) {
		return true
	}

	return false
}

// SetYear gets a reference to the given int64 and assigns it to the Year field.
func (o *UpdateBudgetsRequestBudget) SetYear(v int64) {
	o.Year = &v
}

// GetStartDate returns the StartDate field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetStartDate() time.Time {
	if o == nil || IsNil(o.StartDate) {
		var ret time.Time
		return ret
	}
	return *o.StartDate
}

// GetStartDateOk returns a tuple with the StartDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetStartDateOk() (*time.Time, bool) {
	if o == nil || IsNil(o.StartDate) {
		return nil, false
	}
	return o.StartDate, true
}

// IsSetStartDate returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetStartDate() bool {
	if o != nil && !IsNil(o.StartDate) {
		return true
	}

	return false
}

// SetStartDate gets a reference to the given time.Time and assigns it to the StartDate field.
func (o *UpdateBudgetsRequestBudget) SetStartDate(v time.Time) {
	o.StartDate = &v
}

// GetEndDate returns the EndDate field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetEndDate() time.Time {
	if o == nil || IsNil(o.EndDate) {
		var ret time.Time
		return ret
	}
	return *o.EndDate
}

// GetEndDateOk returns a tuple with the EndDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetEndDateOk() (*time.Time, bool) {
	if o == nil || IsNil(o.EndDate) {
		return nil, false
	}
	return o.EndDate, true
}

// IsSetEndDate returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetEndDate() bool {
	if o != nil && !IsNil(o.EndDate) {
		return true
	}

	return false
}

// SetEndDate gets a reference to the given time.Time and assigns it to the EndDate field.
func (o *UpdateBudgetsRequestBudget) SetEndDate(v time.Time) {
	o.EndDate = &v
}

// GetInterval returns the Interval field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetInterval() string {
	if o == nil || IsNil(o.Interval) {
		var ret string
		return ret
	}
	return *o.Interval
}

// GetIntervalOk returns a tuple with the Interval field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetIntervalOk() (*string, bool) {
	if o == nil || IsNil(o.Interval) {
		return nil, false
	}
	return o.Interval, true
}

// IsSetInterval returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetInterval() bool {
	if o != nil && !IsNil(o.Interval) {
		return true
	}

	return false
}

// SetInterval gets a reference to the given string and assigns it to the Interval field.
func (o *UpdateBudgetsRequestBudget) SetInterval(v string) {
	o.Interval = &v
}

// GetScopeTenantId returns the ScopeTenantId field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetScopeTenantId() int64 {
	if o == nil || IsNil(o.ScopeTenantId) {
		var ret int64
		return ret
	}
	return *o.ScopeTenantId
}

// GetScopeTenantIdOk returns a tuple with the ScopeTenantId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetScopeTenantIdOk() (*int64, bool) {
	if o == nil || IsNil(o.ScopeTenantId) {
		return nil, false
	}
	return o.ScopeTenantId, true
}

// IsSetScopeTenantId returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetScopeTenantId() bool {
	if o != nil && !IsNil(o.ScopeTenantId) {
		return true
	}

	return false
}

// SetScopeTenantId gets a reference to the given int64 and assigns it to the ScopeTenantId field.
func (o *UpdateBudgetsRequestBudget) SetScopeTenantId(v int64) {
	o.ScopeTenantId = &v
}

// GetScopeGroupId returns the ScopeGroupId field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetScopeGroupId() int64 {
	if o == nil || IsNil(o.ScopeGroupId) {
		var ret int64
		return ret
	}
	return *o.ScopeGroupId
}

// GetScopeGroupIdOk returns a tuple with the ScopeGroupId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetScopeGroupIdOk() (*int64, bool) {
	if o == nil || IsNil(o.ScopeGroupId) {
		return nil, false
	}
	return o.ScopeGroupId, true
}

// IsSetScopeGroupId returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetScopeGroupId() bool {
	if o != nil && !IsNil(o.ScopeGroupId) {
		return true
	}

	return false
}

// SetScopeGroupId gets a reference to the given int64 and assigns it to the ScopeGroupId field.
func (o *UpdateBudgetsRequestBudget) SetScopeGroupId(v int64) {
	o.ScopeGroupId = &v
}

// GetScopeCloudId returns the ScopeCloudId field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetScopeCloudId() int64 {
	if o == nil || IsNil(o.ScopeCloudId) {
		var ret int64
		return ret
	}
	return *o.ScopeCloudId
}

// GetScopeCloudIdOk returns a tuple with the ScopeCloudId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetScopeCloudIdOk() (*int64, bool) {
	if o == nil || IsNil(o.ScopeCloudId) {
		return nil, false
	}
	return o.ScopeCloudId, true
}

// IsSetScopeCloudId returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetScopeCloudId() bool {
	if o != nil && !IsNil(o.ScopeCloudId) {
		return true
	}

	return false
}

// SetScopeCloudId gets a reference to the given int64 and assigns it to the ScopeCloudId field.
func (o *UpdateBudgetsRequestBudget) SetScopeCloudId(v int64) {
	o.ScopeCloudId = &v
}

// GetScopeUserId returns the ScopeUserId field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetScopeUserId() int64 {
	if o == nil || IsNil(o.ScopeUserId) {
		var ret int64
		return ret
	}
	return *o.ScopeUserId
}

// GetScopeUserIdOk returns a tuple with the ScopeUserId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetScopeUserIdOk() (*int64, bool) {
	if o == nil || IsNil(o.ScopeUserId) {
		return nil, false
	}
	return o.ScopeUserId, true
}

// IsSetScopeUserId returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetScopeUserId() bool {
	if o != nil && !IsNil(o.ScopeUserId) {
		return true
	}

	return false
}

// SetScopeUserId gets a reference to the given int64 and assigns it to the ScopeUserId field.
func (o *UpdateBudgetsRequestBudget) SetScopeUserId(v int64) {
	o.ScopeUserId = &v
}

// GetCosts returns the Costs field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetCosts() []int64 {
	if o == nil || IsNil(o.Costs) {
		var ret []int64
		return ret
	}
	return o.Costs
}

// GetCostsOk returns a tuple with the Costs field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetCostsOk() ([]int64, bool) {
	if o == nil || IsNil(o.Costs) {
		return nil, false
	}
	return o.Costs, true
}

// IsSetCosts returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetCosts() bool {
	if o != nil && !IsNil(o.Costs) {
		return true
	}

	return false
}

// SetCosts gets a reference to the given []int64 and assigns it to the Costs field.
func (o *UpdateBudgetsRequestBudget) SetCosts(v []int64) {
	o.Costs = v
}

// GetEnabled returns the Enabled field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetEnabled() bool {
	if o == nil || IsNil(o.Enabled) {
		var ret bool
		return ret
	}
	return *o.Enabled
}

// GetEnabledOk returns a tuple with the Enabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetEnabledOk() (*bool, bool) {
	if o == nil || IsNil(o.Enabled) {
		return nil, false
	}
	return o.Enabled, true
}

// IsSetEnabled returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetEnabled() bool {
	if o != nil && !IsNil(o.Enabled) {
		return true
	}

	return false
}

// SetEnabled gets a reference to the given bool and assigns it to the Enabled field.
func (o *UpdateBudgetsRequestBudget) SetEnabled(v bool) {
	o.Enabled = &v
}

// GetForecastType returns the ForecastType field value if set, zero value otherwise.
func (o *UpdateBudgetsRequestBudget) GetForecastType() UpdateBudgetsRequestBudgetForecastType {
	if o == nil || IsNil(o.ForecastType) {
		var ret UpdateBudgetsRequestBudgetForecastType
		return ret
	}
	return *o.ForecastType
}

// GetForecastTypeOk returns a tuple with the ForecastType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateBudgetsRequestBudget) GetForecastTypeOk() (*UpdateBudgetsRequestBudgetForecastType, bool) {
	if o == nil || IsNil(o.ForecastType) {
		return nil, false
	}
	return o.ForecastType, true
}

// IsSetForecastType returns a boolean if a field has been set.
func (o *UpdateBudgetsRequestBudget) IsSetForecastType() bool {
	if o != nil && !IsNil(o.ForecastType) {
		return true
	}

	return false
}

// SetForecastType gets a reference to the given UpdateBudgetsRequestBudgetForecastType and assigns it to the ForecastType field.
func (o *UpdateBudgetsRequestBudget) SetForecastType(v UpdateBudgetsRequestBudgetForecastType) {
	o.ForecastType = &v
}

func (o UpdateBudgetsRequestBudget) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o UpdateBudgetsRequestBudget) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["name"] = o.Name
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.Scope) {
		toSerialize["scope"] = o.Scope
	}
	if !IsNil(o.Period) {
		toSerialize["period"] = o.Period
	}
	if !IsNil(o.Year) {
		toSerialize["year"] = o.Year
	}
	if !IsNil(o.StartDate) {
		toSerialize["startDate"] = o.StartDate
	}
	if !IsNil(o.EndDate) {
		toSerialize["endDate"] = o.EndDate
	}
	if !IsNil(o.Interval) {
		toSerialize["interval"] = o.Interval
	}
	if !IsNil(o.ScopeTenantId) {
		toSerialize["scopeTenantId"] = o.ScopeTenantId
	}
	if !IsNil(o.ScopeGroupId) {
		toSerialize["scopeGroupId"] = o.ScopeGroupId
	}
	if !IsNil(o.ScopeCloudId) {
		toSerialize["scopeCloudId"] = o.ScopeCloudId
	}
	if !IsNil(o.ScopeUserId) {
		toSerialize["scopeUserId"] = o.ScopeUserId
	}
	if !IsNil(o.Costs) {
		toSerialize["costs"] = o.Costs
	}
	if !IsNil(o.Enabled) {
		toSerialize["enabled"] = o.Enabled
	}
	if !IsNil(o.ForecastType) {
		toSerialize["forecastType"] = o.ForecastType
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

type NullableUpdateBudgetsRequestBudget struct {
	value *UpdateBudgetsRequestBudget
	isSet bool
}

func (v NullableUpdateBudgetsRequestBudget) Get() *UpdateBudgetsRequestBudget {
	return v.value
}

func (v *NullableUpdateBudgetsRequestBudget) Set(val *UpdateBudgetsRequestBudget) {
	v.value = val
	v.isSet = true
}

func (v NullableUpdateBudgetsRequestBudget) IsSet() bool {
	return v.isSet
}

func (v *NullableUpdateBudgetsRequestBudget) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUpdateBudgetsRequestBudget(val *UpdateBudgetsRequestBudget) *NullableUpdateBudgetsRequestBudget {
	return &NullableUpdateBudgetsRequestBudget{value: val, isSet: true}
}

func (v NullableUpdateBudgetsRequestBudget) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUpdateBudgetsRequestBudget) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

func (v NullableUpdateBudgetsRequestBudget) UnmarshalMapstructure(data any) (any, error) {
	if err := mapstructDecode(data, &v.value); err != nil {
		return nil, err
	}
	v.isSet = true

	return v, nil
}

func (o *UpdateBudgetsRequestBudget) UnmarshalJSON(data []byte) (err error) {
	return decode(data, &o)
}

// - model_simple.mustache
