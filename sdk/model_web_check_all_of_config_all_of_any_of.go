/*
Morpheus API

Morpheus is a powerful cloud management tool that provides provisioning, monitoring, logging, backups, and application deployment strategies.  This document describes the Morpheus API protocol and the available endpoints. Sections are organized in the same manner as they appear in the Morpheus UI.

API version: 8.0.10
Contact: dev@morpheusdata.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sdk

import (
	"encoding/json"
)

// checks if the WebCheckAllOfConfigAllOfAnyOf type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &WebCheckAllOfConfigAllOfAnyOf{}

// WebCheckAllOfConfigAllOfAnyOf struct for WebCheckAllOfConfigAllOfAnyOf
type WebCheckAllOfConfigAllOfAnyOf struct {
	SshPort              *int64                 `json:"sshPort,omitempty"`
	CheckUser            *string                `json:"checkUser,omitempty"`
	TunnelOn             *string                `json:"tunnelOn,omitempty"`
	TextCheckOn          *string                `json:"textCheckOn,omitempty"`
	CheckPassword        *string                `json:"checkPassword,omitempty"`
	SshHost              *string                `json:"sshHost,omitempty"`
	SshUser              *string                `json:"sshUser,omitempty"`
	WebTextMatch         *string                `json:"webTextMatch,omitempty"`
	CheckPasswordHash    *string                `json:"checkPasswordHash,omitempty"`
	AdditionalProperties map[string]interface{} `json:",remain"`
}

type _WebCheckAllOfConfigAllOfAnyOf WebCheckAllOfConfigAllOfAnyOf

// NewWebCheckAllOfConfigAllOfAnyOf instantiates a new WebCheckAllOfConfigAllOfAnyOf object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewWebCheckAllOfConfigAllOfAnyOf() *WebCheckAllOfConfigAllOfAnyOf {
	this := WebCheckAllOfConfigAllOfAnyOf{}
	return &this
}

// NewWebCheckAllOfConfigAllOfAnyOfWithDefaults instantiates a new WebCheckAllOfConfigAllOfAnyOf object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewWebCheckAllOfConfigAllOfAnyOfWithDefaults() *WebCheckAllOfConfigAllOfAnyOf {
	this := WebCheckAllOfConfigAllOfAnyOf{}
	return &this
}

// GetSshPort returns the SshPort field value if set, zero value otherwise.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetSshPort() int64 {
	if o == nil || IsNil(o.SshPort) {
		var ret int64
		return ret
	}
	return *o.SshPort
}

// GetSshPortOk returns a tuple with the SshPort field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetSshPortOk() (*int64, bool) {
	if o == nil || IsNil(o.SshPort) {
		return nil, false
	}
	return o.SshPort, true
}

// IsSetSshPort returns a boolean if a field has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) IsSetSshPort() bool {
	if o != nil && !IsNil(o.SshPort) {
		return true
	}

	return false
}

// SetSshPort gets a reference to the given int64 and assigns it to the SshPort field.
func (o *WebCheckAllOfConfigAllOfAnyOf) SetSshPort(v int64) {
	o.SshPort = &v
}

// GetCheckUser returns the CheckUser field value if set, zero value otherwise.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetCheckUser() string {
	if o == nil || IsNil(o.CheckUser) {
		var ret string
		return ret
	}
	return *o.CheckUser
}

// GetCheckUserOk returns a tuple with the CheckUser field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetCheckUserOk() (*string, bool) {
	if o == nil || IsNil(o.CheckUser) {
		return nil, false
	}
	return o.CheckUser, true
}

// IsSetCheckUser returns a boolean if a field has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) IsSetCheckUser() bool {
	if o != nil && !IsNil(o.CheckUser) {
		return true
	}

	return false
}

// SetCheckUser gets a reference to the given string and assigns it to the CheckUser field.
func (o *WebCheckAllOfConfigAllOfAnyOf) SetCheckUser(v string) {
	o.CheckUser = &v
}

// GetTunnelOn returns the TunnelOn field value if set, zero value otherwise.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetTunnelOn() string {
	if o == nil || IsNil(o.TunnelOn) {
		var ret string
		return ret
	}
	return *o.TunnelOn
}

// GetTunnelOnOk returns a tuple with the TunnelOn field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetTunnelOnOk() (*string, bool) {
	if o == nil || IsNil(o.TunnelOn) {
		return nil, false
	}
	return o.TunnelOn, true
}

// IsSetTunnelOn returns a boolean if a field has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) IsSetTunnelOn() bool {
	if o != nil && !IsNil(o.TunnelOn) {
		return true
	}

	return false
}

// SetTunnelOn gets a reference to the given string and assigns it to the TunnelOn field.
func (o *WebCheckAllOfConfigAllOfAnyOf) SetTunnelOn(v string) {
	o.TunnelOn = &v
}

// GetTextCheckOn returns the TextCheckOn field value if set, zero value otherwise.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetTextCheckOn() string {
	if o == nil || IsNil(o.TextCheckOn) {
		var ret string
		return ret
	}
	return *o.TextCheckOn
}

// GetTextCheckOnOk returns a tuple with the TextCheckOn field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetTextCheckOnOk() (*string, bool) {
	if o == nil || IsNil(o.TextCheckOn) {
		return nil, false
	}
	return o.TextCheckOn, true
}

// IsSetTextCheckOn returns a boolean if a field has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) IsSetTextCheckOn() bool {
	if o != nil && !IsNil(o.TextCheckOn) {
		return true
	}

	return false
}

// SetTextCheckOn gets a reference to the given string and assigns it to the TextCheckOn field.
func (o *WebCheckAllOfConfigAllOfAnyOf) SetTextCheckOn(v string) {
	o.TextCheckOn = &v
}

// GetCheckPassword returns the CheckPassword field value if set, zero value otherwise.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetCheckPassword() string {
	if o == nil || IsNil(o.CheckPassword) {
		var ret string
		return ret
	}
	return *o.CheckPassword
}

// GetCheckPasswordOk returns a tuple with the CheckPassword field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetCheckPasswordOk() (*string, bool) {
	if o == nil || IsNil(o.CheckPassword) {
		return nil, false
	}
	return o.CheckPassword, true
}

// IsSetCheckPassword returns a boolean if a field has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) IsSetCheckPassword() bool {
	if o != nil && !IsNil(o.CheckPassword) {
		return true
	}

	return false
}

// SetCheckPassword gets a reference to the given string and assigns it to the CheckPassword field.
func (o *WebCheckAllOfConfigAllOfAnyOf) SetCheckPassword(v string) {
	o.CheckPassword = &v
}

// GetSshHost returns the SshHost field value if set, zero value otherwise.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetSshHost() string {
	if o == nil || IsNil(o.SshHost) {
		var ret string
		return ret
	}
	return *o.SshHost
}

// GetSshHostOk returns a tuple with the SshHost field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetSshHostOk() (*string, bool) {
	if o == nil || IsNil(o.SshHost) {
		return nil, false
	}
	return o.SshHost, true
}

// IsSetSshHost returns a boolean if a field has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) IsSetSshHost() bool {
	if o != nil && !IsNil(o.SshHost) {
		return true
	}

	return false
}

// SetSshHost gets a reference to the given string and assigns it to the SshHost field.
func (o *WebCheckAllOfConfigAllOfAnyOf) SetSshHost(v string) {
	o.SshHost = &v
}

// GetSshUser returns the SshUser field value if set, zero value otherwise.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetSshUser() string {
	if o == nil || IsNil(o.SshUser) {
		var ret string
		return ret
	}
	return *o.SshUser
}

// GetSshUserOk returns a tuple with the SshUser field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetSshUserOk() (*string, bool) {
	if o == nil || IsNil(o.SshUser) {
		return nil, false
	}
	return o.SshUser, true
}

// IsSetSshUser returns a boolean if a field has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) IsSetSshUser() bool {
	if o != nil && !IsNil(o.SshUser) {
		return true
	}

	return false
}

// SetSshUser gets a reference to the given string and assigns it to the SshUser field.
func (o *WebCheckAllOfConfigAllOfAnyOf) SetSshUser(v string) {
	o.SshUser = &v
}

// GetWebTextMatch returns the WebTextMatch field value if set, zero value otherwise.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetWebTextMatch() string {
	if o == nil || IsNil(o.WebTextMatch) {
		var ret string
		return ret
	}
	return *o.WebTextMatch
}

// GetWebTextMatchOk returns a tuple with the WebTextMatch field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetWebTextMatchOk() (*string, bool) {
	if o == nil || IsNil(o.WebTextMatch) {
		return nil, false
	}
	return o.WebTextMatch, true
}

// IsSetWebTextMatch returns a boolean if a field has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) IsSetWebTextMatch() bool {
	if o != nil && !IsNil(o.WebTextMatch) {
		return true
	}

	return false
}

// SetWebTextMatch gets a reference to the given string and assigns it to the WebTextMatch field.
func (o *WebCheckAllOfConfigAllOfAnyOf) SetWebTextMatch(v string) {
	o.WebTextMatch = &v
}

// GetCheckPasswordHash returns the CheckPasswordHash field value if set, zero value otherwise.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetCheckPasswordHash() string {
	if o == nil || IsNil(o.CheckPasswordHash) {
		var ret string
		return ret
	}
	return *o.CheckPasswordHash
}

// GetCheckPasswordHashOk returns a tuple with the CheckPasswordHash field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) GetCheckPasswordHashOk() (*string, bool) {
	if o == nil || IsNil(o.CheckPasswordHash) {
		return nil, false
	}
	return o.CheckPasswordHash, true
}

// IsSetCheckPasswordHash returns a boolean if a field has been set.
func (o *WebCheckAllOfConfigAllOfAnyOf) IsSetCheckPasswordHash() bool {
	if o != nil && !IsNil(o.CheckPasswordHash) {
		return true
	}

	return false
}

// SetCheckPasswordHash gets a reference to the given string and assigns it to the CheckPasswordHash field.
func (o *WebCheckAllOfConfigAllOfAnyOf) SetCheckPasswordHash(v string) {
	o.CheckPasswordHash = &v
}

func (o WebCheckAllOfConfigAllOfAnyOf) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o WebCheckAllOfConfigAllOfAnyOf) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.SshPort) {
		toSerialize["sshPort"] = o.SshPort
	}
	if !IsNil(o.CheckUser) {
		toSerialize["checkUser"] = o.CheckUser
	}
	if !IsNil(o.TunnelOn) {
		toSerialize["tunnelOn"] = o.TunnelOn
	}
	if !IsNil(o.TextCheckOn) {
		toSerialize["textCheckOn"] = o.TextCheckOn
	}
	if !IsNil(o.CheckPassword) {
		toSerialize["checkPassword"] = o.CheckPassword
	}
	if !IsNil(o.SshHost) {
		toSerialize["sshHost"] = o.SshHost
	}
	if !IsNil(o.SshUser) {
		toSerialize["sshUser"] = o.SshUser
	}
	if !IsNil(o.WebTextMatch) {
		toSerialize["webTextMatch"] = o.WebTextMatch
	}
	if !IsNil(o.CheckPasswordHash) {
		toSerialize["checkPasswordHash"] = o.CheckPasswordHash
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

type NullableWebCheckAllOfConfigAllOfAnyOf struct {
	value *WebCheckAllOfConfigAllOfAnyOf
	isSet bool
}

func (v NullableWebCheckAllOfConfigAllOfAnyOf) Get() *WebCheckAllOfConfigAllOfAnyOf {
	return v.value
}

func (v *NullableWebCheckAllOfConfigAllOfAnyOf) Set(val *WebCheckAllOfConfigAllOfAnyOf) {
	v.value = val
	v.isSet = true
}

func (v NullableWebCheckAllOfConfigAllOfAnyOf) IsSet() bool {
	return v.isSet
}

func (v *NullableWebCheckAllOfConfigAllOfAnyOf) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableWebCheckAllOfConfigAllOfAnyOf(val *WebCheckAllOfConfigAllOfAnyOf) *NullableWebCheckAllOfConfigAllOfAnyOf {
	return &NullableWebCheckAllOfConfigAllOfAnyOf{value: val, isSet: true}
}

func (v NullableWebCheckAllOfConfigAllOfAnyOf) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableWebCheckAllOfConfigAllOfAnyOf) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

func (v NullableWebCheckAllOfConfigAllOfAnyOf) UnmarshalMapstructure(data any) (any, error) {
	if err := mapstructDecode(data, &v.value); err != nil {
		return nil, err
	}
	v.isSet = true

	return v, nil
}
func (o *WebCheckAllOfConfigAllOfAnyOf) UnmarshalJSON(data []byte) (err error) {
	return decode(data, &o)
}

// - model_simple.mustache
